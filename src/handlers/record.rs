...
pub async fn get_user_records(data: web::Data<State>, uid: web::Path<i32>) -> HttpResponse {
    match User::get_by_id(&data.db.lock().unwrap(), *uid).await {
        Ok(Some(user)) => {
            match User::get_all_records(&data.db.lock().unwrap(), user.id.unwrap()).await {
...
/// TODO implement
pub async fn update_user_record(path: web::Path<i32>, data: web::Data<State>) -> HttpResponse {
