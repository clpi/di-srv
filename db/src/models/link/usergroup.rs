...
impl UserGroupLink {
    ...
    pub async fn insert(self, db: &Db) -> sqlx::Result<u32> {
        let res = sqlx::query(
            "INSERT INTO UserGroupLinks (uid, gid, group_role, created_at)
            VALUES ($1, $2, $3) RETURNING id")
            .bind(&self.uid)
            .bind(&self.gid)
            .bind(&self.group_role)
        ...
        -> sqlx::Result<Vec<User>> 
    {
        let res: Vec<User> = sqlx::query_as::<Postgres, User>(
           "SELECT u.id, u.username, u.email, u.created_at, g.name, ug.group_role, ug.created_at,
            FROM Users u INNER JOIN UserGroupLinks ug ON u.id=ug.uid
        ...
        -> sqlx::Result<Vec<Group>> 
    {
        ...
        "SELECT g.id, g.name, g.created_at, g.status, u.username, 
            ...
            FROM Groups g INNER JOIN UserGroupLinks ug ON g.id=ug.gid
...
impl From<(User, Group)> for UserGroupLink {
